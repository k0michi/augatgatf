#include "web_audio/detail/upsampler2x.hh"

#include "web_audio/detail/math_helper.hh"
#include "web_audio/detail/wave_processing.hh"

namespace web_audio::detail {
Upsampler2x::Upsampler2x(std::size_t blockSize) : inputBlockSize_(blockSize) {
  filterCoefficients_ = {
  ...
  outputBlockSize_ = inputBlockSize_ * 2;
...
void Upsampler2x::process(const std::vector<float> &input,
